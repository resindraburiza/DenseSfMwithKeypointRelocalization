function at_generate_matches4colmap(ii,imgs,feat_pth,match_pth,min_match,top_netvlad,graph,score,finerlayer,coarselayerlevel,finelayerlevel)
...
if ~isempty(score)
    score = score(ii,:);
    [sorted, sortedIdx] = sort(score,'descend');
end
...
if ~exist(f1cnn_matchfn,'file')
    ...
    if isempty(score)
        ...
        for jj = sortedIdx(2:top_netvlad+1)
            if jj<ii
                continue;
            end
            [head2, tail2] = str_cut(imgs{jj},'.');
